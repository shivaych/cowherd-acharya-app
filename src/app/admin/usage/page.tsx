  ...
  return (
    <div>
      ...
      <p className="text-[10.5px] text-muted italic mt-6">
        Costs are estimates from published provider rates. For billing truth, check Google Cloud consoles.
      </p>
    </div>
  );
}
      ...
      <div>
        ...
        {source && (
          <p className="font-mono text-[10px] tracking-[0.18em] uppercase text-muted mt-1">
            source: {source}
            {source === 'supabase' ? ' · cowherd_ai_usage' : source === 'file' ? ' · logs/ai/calls.jsonl' : ''}
          </p>
        )}
      </div>
      <button
