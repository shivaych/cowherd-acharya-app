...
export async function GET(req: NextRequest) {
  ...
  const [{ data: progress }, { data: quizzes }] = await Promise.all([
    db.from("cowherd_progress").select("learner_id, completed").in("learner_id", learnerIds),
    db.from("cowherd_quiz_attempts").select("learner_id, score, total").in("learner_id", learnerIds),
  ]);
