...
$body$Wash your hands before touching the animal. Tie the animal securely — front and back leg. Clean the udder with a clean damp cloth, wiping each teat from base to tip. Discard the first 2-3 squirts from each teat to a strip cup — this forestripping detects mastitis (look for clots, wateriness, or blood) and removes the bacterial-rich first milk. The pre-milking stimulation from cleaning and forestripping triggers oxytocin release, which causes milk let-down. If you skip this, the cow will not give her full milk. The milking area must be clean and calm — noise and disturbance block oxytocin and reduce yield.$body$,
'published'
...
$body$Use the full-hand squeeze method — wrap all four fingers and thumb around the teat, close from top to bottom in a wave motion. Do not pull or strip downward using thumb and forefinger alone — this causes teat damage and mastitis over time. Milk all four quarters completely — incomplete milking causes mastitis and reduces future production. After the last quarter is done, apply iodine teat dip (0.5% solution) to each teat immediately. Do not wipe it off — let it air dry. This kills bacteria that enter the open teat canal after milking. Teat dipping is the single most effective mastitis prevention measure on any dairy farm. Store milk in a clean, covered vessel and transfer to cold storage within 30 minutes.$body$,
'published'
FROM acharya_cowherd.crs_sections s
JOIN acharya_cowherd.crs_modules m ON s.module_id = m.id
